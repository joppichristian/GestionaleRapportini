      ...
      if($stmt->num_rows == 1) {
				//if(!possibile_attacco($user_id, $mysqli_generale)){
					if($db_password == $password){
						$qry = "SELECT * FROM Azienda_utente AU JOIN utente_azienda AS ua ON AU.ID_Utente = ua.id_utente JOIN Azienda_cliente AC ON ua.id_azienda = AC.ID_azienda WHERE username='".$username."' AND password='".$db_password."' AND attivo = 1;";
