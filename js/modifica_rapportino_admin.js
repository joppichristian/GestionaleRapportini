...
function setVarRapp(index_rapp){
  ...
  inizio = (det_json[index_rapp]['inizio']).split(' ')[1].substr(0, 5);
  fine = (det_json[index_rapp]['fine']).split(' ')[1].substr(0, 5);
...
function mod_controlloRapCliente(){
 ...
 for(var i=0;i<start.length;i++){
	...
	if(result && start.length > 1) {
		var g = $("#mod_giorno_dd").val()+"-"+$("#mod_giorno_mm").val()+"-"+$("#mod_giorno_yy").val();
		var d = id_utente;
		var c = cliente;
	...
	else
	{
		...
		Materialize.toast("Rapportino modificato!",2000,"",function(){$("#modal1").closeModal();})
