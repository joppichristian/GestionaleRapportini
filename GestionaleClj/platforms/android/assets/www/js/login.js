...
function controlla_login(user,psw){
  ...
  var query = "http://gestionaleclj.com/script_php/login.php?"+stringaR ;
  $.ajax({
      url: query,
      async:false,
      success: function(data) {
        json = data;
          ...
          for(var i = 0; i < data.length; i++) {
			...
			setCookie("nome_utente", username, 30);
            Materialize.toast('Login avvenuto con successo!', 2000,'',function(){window.location.href = 'dashboard.html?nomeDB=' +nomedb+'&classe_privilegi='+admin});
          }else{
